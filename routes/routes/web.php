...
route::prefix('')->middleware(['confirmcode',\App\Http\Middleware\ChangeRole::class,'throttle:trynal',\App\Http\Middleware\isagent::class])->group(function(){
    ...
    route::post('folgepost/{id}',function ($id,Request $req){
        $id = Crypt::decrypt($id);
        $id = $id / 1244;
        lead::find($id)->update(array('appointment_date' => $req->ndate));
        Admins::role(['salesmanager'])->get()->each(function($item){
           $item->notify(new SendNotificationn('<a href="' . route('appointments') . '"> Ein Berater hat einen Termin verschoben </a>'));
        });
    ...
    route::get('updatelead/{id}',[UserController::class,'updatelead'])->name('updatelead');
    ...
    route::get('accepttask/{id?}',[TasksController::class,'accepttask'])->name('accepttask')->middleware('role:admin|salesmanager|backoffice');

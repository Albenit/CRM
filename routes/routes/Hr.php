...
route::post('createAbsence',[HumanResourcesController::class,'createAbsence'])->name('createAbsence');
    ...
    route::get('hr_view', function (Request $request){
        ...
        foreach (\App\Models\CostumerProduktGrundversicherung::where('admin_id',auth()->id())->whereIn('status_PG',['Provisionert'])->get() as $produkt){
            $contracts->push(['val' => $produkt->total_commisions_PG,'field' => 'Grund','company' => $produkt->society_PG]);
        }
        foreach (\App\Models\CostumerProduktZusatzversicherung::where('admin_id',auth()->id())->whereIn('status_PZ',['Provisionert'])->get() as $produkt){
            $contracts->push(['val' => $produkt->total_commisions_PZ,'field' => 'Zusat','company' =>$produkt->society_PZ]);
        }
        foreach (\App\Models\CostumerProduktRechtsschutz::where('admin_id',auth()->id())->whereIn('status_PR',['Provisionert'])->get() as $produkt){
            $contracts->push(['val' => $produkt->total_commisions_PR,'field' => 'Ru','company' =>$produkt->society_PR]);
        }
        $rroga = 0;
        foreach ($contracts as $contract){
            $rroga += getsalary($contract['company'],$contract['field'],$contract['val']);
        }
        ...
        if($user->hasRole('admin')  || $user->hasRole('backoffice'))
        {
            ...
            $absences = Absence::whereHas('admin')->with('admin')->orderBy('created_at', 'desc')->where('type',0)->get();
            $date_to = new DateTime(Carbon::now());
            $bestellungen = bestellungen::orderBy('created_at', 'desc')->get();
        } else{
            $date_in =  new DateTime($date_in);
            $absences = Absence::whereHas('admin')->with('admin')->where('employee_id',auth()->user()->id)->orderBy('created_at', 'desc')->where('type',0)->get();
            $date_to = new DateTime(Carbon::now());
            $bestellungen = bestellungen::where('employee_id',auth()->user()->id)->orderBy('created_at', 'desc')->get();
        }
        if ($request->searchEmployes == null){
            $admins = \App\Models\Admins::with('personaldata')->get();
        }else{
...
if(auth()->user()->hasRole('fs')){
        return view('hr',['personalDatas' => $personalDatas,'bestellungen' => $bestellungen,'admins' => $admins,'personalData' => $personalData,'bankInfo' => $bankInfo])->with('admins',$admins)->with('absences',$absences)->with('date_in',$date_in)->with('date_to',$date_to)->with('rroga',$rroga);
}
else{
    return view('hr',['personalDatas' => $personalDatas,'bestellungen' => $bestellungen,'admins' => $admins,'personalData' => $personalData,'bankInfo' => $bankInfo])->with('admins',$admins)->with('absences',$absences)->with('date_in',$date_in)->with('date_to',$date_to)->with('rroga',$rroga);
}
    ...
    route::get('employeProfile/{id}',function ($id){
        ...
        return view('personalData',compact('personalData','bankInfo','person','rroga'));
